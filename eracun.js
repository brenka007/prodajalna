      ...
      strankaIzRacuna(polja.seznamRacunov, function(stranka) {
        pesmiIzRacuna(polja.seznamRacunov, function(pesmi) {
          if (!pesmi) {
            ...
            saj v košarici ni nobene pesmi!</p>");
          } else {
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    try {
      ...
      stmt.finalize();
      message = "Stranka uspešno registrirana.";
    } catch (err) {
      napaka2 = true;
      message = "Med registracijo nove stranke je prišlo do napake. Preverite vnešene podatke ter poskusite znova.";
    }
  
    odgovor.redirect(zahteva.get('referer'));
  });
})
...
streznik.post('/odjava', function(zahteva, odgovor) {
    zahteva.session.stranka = undefined;
    odgovor.redirect('/prijava') 
})
